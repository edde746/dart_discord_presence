...
class DiscordButton {
  ...
  const DiscordButton({required this.label, required this.url})
    : assert(label.length <= 32, 'Button label must be 32 characters or less');
  ...
  DiscordButton copyWith({String? label, String? url}) {
    return DiscordButton(label: label ?? this.label, url: url ?? this.url);
  }
}
